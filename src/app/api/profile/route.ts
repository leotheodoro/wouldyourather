...
export async function POST(req: NextRequest) {
  ...
  if (!parsed.success || parsed.data.history.length === 0) {
    return new Response(
      JSON.stringify({ error: "History must have at least one item" }),
      {
        status: 400,
  ...
  try {
    const result = await generateText({
      ...
      messages: [{ role: "user", content: buildProfilePrompt(history) }],
    });
    ...
    const message =
      err instanceof Error ? err.message : "MNEMOSYNE não respondeu";
    ...
    async start(controller) {
      ...
      try {
        for (const char of textToStream) {
          controller.enqueue(
            enc.encode(`data: ${JSON.stringify({ token: char })}\n\n`),
          );
          await new Promise<void>((r) => setTimeout(r, 12));
        }
        controller.enqueue(
          enc.encode(`data: ${JSON.stringify({ done: true, profile })}\n\n`),
        );
      } catch {
        controller.enqueue(
          enc.encode(
            `data: ${JSON.stringify({ done: true, error: true })}\n\n`,
          ),
        );
      } finally {
        controller.close();
